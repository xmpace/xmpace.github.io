...
App 与长连服务器之间的包有什么特征呢？首先肯定是 TCP ，文中虽然有说还有一条 UDP 通道，但我觉得这不靠谱，应该不是指的长连使用 UDP。其次，由于是长连，所以客户端是不会主动去断开这个连接的，也就是说是不会发送 FIN 包的（但不排除刚打开时跑马测速后会断开）。有了这两个特征，我们就可以开始筛选 IP 了。电脑打开 Wireshark ，开个热点给手机，找出符合这两个特征的 IP 后屏蔽就可以了。笔者的路由器刷的 Padavan 系统，可以直接用 iptables 屏蔽，所以就在路由器上屏蔽了。
